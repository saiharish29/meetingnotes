...
export default function TranscriptInput({ onGenerate }: TranscriptInputProps) {
  ...
  }, [meetingTitle]); // eslint-disable-line react-hooks/exhaustive-deps
              ...
              <div className="animate-fade-in">
                ...
                {transcript && (
                  <p className="mt-2 text-xs text-surface-400">
                    {transcript.split('\n').filter(Boolean).length} lines · {transcript.length.toLocaleString()} characters
                  </p>
                )}
              </div>
            ) : (
              <div className="animate-fade-in">
                ...
                >
                  ...
                  <p className="text-xs text-surface-400 mt-1">
                    {fileName ? `${transcript.split('\n').filter(Boolean).length} lines loaded` : 'or click to browse — .txt or .md'}
                  </p>
                </div>

                {transcript && tab === 'upload' && (
                  <div className="mt-3 p-3 rounded-xl bg-surface-50 border border-surface-100">
                    <p className="text-xs font-medium text-surface-600 mb-1">Preview (first 3 lines):</p>
        ...
        {attendees.length > 0 && (
          <div className="bg-white rounded-2xl border border-surface-100 shadow-sm p-5 animate-slide-up">
            ...
            <div className="flex flex-wrap gap-2">
              {attendees.map(name => (
                <span key={name} className="px-3 py-1 rounded-full bg-brand-50 text-brand-700 text-xs font-medium border border-brand-100">
                  {name}
                </span>
              ))}
            </div>
